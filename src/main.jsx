...
function App() {
  ...
  useEffect(() => {
    Promise.all([getProducts(), getServices()]).then(([productsResult, servicesResult]) => {
      if (productsResult.ok) setStoreProducts(productsResult.products.map((product) => ({ ...product, price: `Ugx ${Number(product.amount).toLocaleString()}`, items: product.description.split('\n').filter(Boolean) })));
      if (servicesResult.ok && servicesResult.services.length) setStoreServices(servicesResult.services.map((service) => ({ ...service, price: `Ugx ${Number(service.amount).toLocaleString()}`, items: service.description.split('\n').filter(Boolean) })));
    });
  }, []);
  ...
  return <div className="site-shell">
    ...
    <main id="top">
      ...
      <section className="pricing section" id="pricing"><div className="section-label">/ Services</div><div className="section-heading"><h2>Choose the right <em>service package.</em></h2><p>Service pricing is managed independently from physical products. Add a package to your request and we will confirm installation and payment.</p></div><div className="pricing-grid">{storeServices.map((pack, index) => <article className={`price-card ${index === 1 ? 'featured' : ''}`} key={pack.id || pack.name}><div className="price-card-top"><span>{pack.type}</span><span>0{index + 1}</span></div><h3>{pack.name}</h3><ul>{pack.items.map((item) => <li key={item}>{item}</li>)}</ul><div className="price-bottom"><strong>{pack.price}</strong><button className="add-button" onClick={() => addToCart({ ...pack, amount: pack.amount })} aria-label={`Add ${pack.name} to cart`}><Plus size={19} /> Add</button></div></article>)}</div></section>
      {storeProducts.length > 0 && <section className="product-catalog section"><div className="section-label">/ Products</div><div className="section-heading"><h2>Equipment for <em>the work ahead.</em></h2><p>Physical products and equipment are managed separately from service packages.</p></div><div className="product-store-grid">{storeProducts.map((product) => <article className="store-product" key={product.id}><img src={product.image_url || 'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?auto=format&fit=crop&w=800&q=80'} alt="" /><div><span className="order-date">{product.type}</span><h3>{product.name}</h3><strong>{product.price}</strong><button className="add-button" onClick={() => addToCart(product)}>Add to cart</button></div></article>)}</div></section>}
