...
drop policy if exists "Public can view active services" on public.service_packages;
create policy "Public can view active services"
  on public.service_packages for select to anon, authenticated
  using (is_active = true or public.is_admin());

drop policy if exists "Admins can create services" on public.service_packages;
create policy "Admins can create services"
  on public.service_packages for insert to authenticated
  with check (public.is_admin());

drop policy if exists "Admins can update services" on public.service_packages;
create policy "Admins can update services"
  on public.service_packages for update to authenticated
  using (public.is_admin()) with check (public.is_admin());

drop policy if exists "Admins can delete services" on public.service_packages;
